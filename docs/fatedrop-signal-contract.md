# FateDrop canonical signal contract

This is the final product and business definition for FateDrop network lifecycle terminology. Do not reinterpret these names in app, web, Discord, push, Companion, marketing or future business plans without an explicit product decision.

## WHISPER — something moved
Pre-event product/catalogue intelligence. Whisper is triggered by meaningful product-side changes before confirmed live availability: catalogue additions, new listings, preorder/coming-soon changes, stock metadata changes, product-page changes, or other relevant catalogue movement. A Whisper says: **something may be coming; watch this product.**

Whisper is NOT confirmed queue/security activity and is NOT confirmed purchasable stock.

## ECHO — get ready
Readiness intelligence from retailer/site behaviour rather than ordinary catalogue stock transitions. Echo covers meaningful traffic, queue, waiting-room, security/challenge, access-control or similar infrastructure changes that increase confidence a drop/event may be imminent. An Echo says: **get ready; get into position / queue.**

Echo is NOT confirmed purchasable stock.

## MANIFESTED — go now
Confirmed live/purchasable availability. This includes first availability and verified restocks. A Manifested signal says: **stock is live; act now and buy if you want it.**

## VANISHED — it is gone
Previously confirmed purchasable availability is no longer verified. A Vanished signal says: **stock appears gone / sold out; you may have missed it.** The product remains navigable so FateDrop can show current status and alternatives.

## Lifecycle
**WHISPER → ECHO → MANIFESTED → VANISHED**

Not every event must pass through every stage. For example, stock can Manifest without a prior Echo if no infrastructure precursor is observed. A catalogue change can Whisper without ever Manifesting. The words describe evidence states, not a mandatory timed sequence.

## Cross-platform invariant
All four states use the same canonical product identity whenever a product can be identified. Alerts must be actionable rather than dead notifications:
- Whisper → product intelligence/search/comparison context.
- Echo → product + retailer readiness/queue context.
- Manifested → product + current retailer offer / buy action.
- Vanished → product current state + alternative known offers.

## Price truth invariant
RRP/markup context may accompany any product-linked signal only when FateDrop has an authoritative RRP for that canonical identity. Never guess an RRP. Item price vs RRP remains valid even when delivery is unknown; delivery-aware True Price is additional evidence, not a prerequisite for markup percentage.

## Companion intent
Whisper = notice/scan. Echo = anticipation/readiness. Manifested = strongest action/victory state. Vanished = missed/subdued state. Preserve alert-specific animation mappings where already assigned.

## Business-plan summary
FateDrop's differentiating alert network is a four-stage intelligence ladder: it notices product movement early (Whisper), detects retailer infrastructure signs that a drop may be imminent (Echo), confirms when stock is genuinely purchasable (Manifested), and records when that availability disappears (Vanished). The same product identity links the collector from intelligence to action and then to alternatives, while evidence-backed RRP pricing explains whether an offer is actually good value.
