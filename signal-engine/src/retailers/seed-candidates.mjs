...
const candidates = deduplicateRetailerCandidates(retailerDiscoverySeeds);
const results = [];
for (const candidate of candidates) {
  const saved = await registry.upsert(candidate);
  results.push({ id: saved.id, name: saved.name, countryCode: saved.countryCode, shipsToUk: saved.delivery.shipsToUk, state: saved.state, verification: saved.verification, adapterType: saved.adapterType });
}
