...
const NON_PRODUCT = /\bsingle\b(?![\s-]+(?:booster[\s-]+)?packs?\b)|code card|sleeve|\bbinder\b|play\s?mat|toploader|graded|\bpsa\b|\bcgc\b|\bbgs\b/i;
...
function travellingManUk(enabled) {
  return {
    ...
    officialRrpSource: false,
    include: SEALED_PRODUCT,
    exclude: /\bsingle\b(?![\s-]+(?:booster[\s-]+)?packs?\b)|code card|sleeve|\bbinder\b|play\s?mat|toploader|graded|\bpsa\b|\bcgc\b|\bbgs\b|portfolio|deck box/i,
  };
}
