  ...
  async saveScan({ retailer, products, offers, observations, signals, completedAt, health }) {
    ...
    try {
      await runTransactionWithRetry(client, async () => {
        ...
        await bulkJson(client, `INSERT INTO fatedrop_product_identities (id,tcg,canonical_key,title,product_type,language_code,region_code,official_rrp_pence,rrp_source,rrp_verified_at,updated_at)
          SELECT x->>'id',x->>'tcg',x->>'canonicalKey',x->>'title',NULLIF(x->>'productType',''),NULLIF(x->>'languageCode',''),NULLIF(x->>'regionCode',''),NULLIF(x->>'officialRrpPence','')::bigint,NULLIF(x->>'rrpSource',''),NULLIF(x->>'rrpObservedAt','')::bigint,(x->>'updatedAt')::bigint
          FROM jsonb_array_elements($1::jsonb) x
          ON CONFLICT (tcg,canonical_key) DO UPDATE SET title=EXCLUDED.title, product_type=COALESCE(EXCLUDED.product_type,fatedrop_product_identities.product_type), language_code=COALESCE(EXCLUDED.language_code,fatedrop_product_identities.language_code), region_code=COALESCE(EXCLUDED.region_code,fatedrop_product_identities.region_code), official_rrp_pence=COALESCE(EXCLUDED.official_rrp_pence,fatedrop_product_identities.official_rrp_pence), rrp_source=COALESCE(EXCLUDED.rrp_source,fatedrop_product_identities.rrp_source), rrp_verified_at=COALESCE(EXCLUDED.rrp_verified_at,fatedrop_product_identities.rrp_verified_at), updated_at=GREATEST(EXCLUDED.updated_at,fatedrop_product_identities.updated_at)`, orderedProducts);
