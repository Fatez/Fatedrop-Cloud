...
async function reconcileCuratedLocalIntel() {
  ...
  try {
    ...
    console.log("[signal-engine] Local Radar curated incoming intelligence", {
      status: outcome.status,
    ...
    console.error("[signal-engine] Local Radar curated incoming intelligence failed", { error: String(error?.message || error) });
  } finally {
...
if (env.databaseUrl) {
  ...
  const curatedIntelTimer = setTimeout(() => { void reconcileCuratedLocalIntel(); }, CURATED_LOCAL_INTEL_START_DELAY_MS);
  curatedIntelTimer.unref();
