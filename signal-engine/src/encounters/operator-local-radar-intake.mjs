...
export async function pollOperatorIssues({ store = operatorStore, fetchImpl = fetch, now = Date.now() } = {}) {
...
if (env.databaseUrl && env.store === "postgres") {
  const timer = setTimeout(() => { void pollOperatorIssues(); }, POLL_START_DELAY_MS);
  timer.unref();
  setInterval(() => { void pollOperatorIssues(); }, POLL_INTERVAL_MS).unref();
}
