...
export async function handleFateCollectors(req,res,{
  ...
  try{
    ...
    if(req.method==='POST'&&url.pathname===CONFIRM_PATH){
      ...
      const result=await confirmCollectrImportFromStore(store,{
        userId:user.id,
    ...
    if(error?.code==='IMPORT_PREVIEW_CHANGED'||error?.code==='IMPORT_STATE_CHANGED'||error?.code==='IMPORT_PREVIEW_TRUNCATED'){
      fail(res,409,error.code,error.message);return true;
    }
