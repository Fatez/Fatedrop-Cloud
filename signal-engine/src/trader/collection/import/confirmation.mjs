...
import { collectrTokenMatchesCsv, makeCollectrImportBatchKey } from './confirmation-token.mjs';
...
export async function confirmCollectrImportFromStore(store,{userId,csvText,confirmationToken,confirmed=false,existingItemLimit=2000}={}){
  ...
  if(token!==preview.confirmationToken){
    ...
    if(sources.some((source)=>source.importBatchKey===batchKey))return resultSummary(preview,null,{duplicate:true,writesPerformed:false,batchKey});
  ...
  if(preview.scale?.mayBeTruncated===true)throw taggedError('IMPORT_PREVIEW_TRUNCATED','The collection is larger than the safe preview window. Increase the bounded import read before confirming.');
  const actionable=preview.actions.creates.length+preview.actions.updates.length+preview.actions.unchanged.length;
  if(actionable===0)return resultSummary(preview,{created:[],updated:[],unchanged:[]},{duplicate:false,writesPerformed:false,batchKey});
  const applied=await applyCollectionImportPlanInStore(store,{userId:ownerId,plan:preview.actions,importBatchKey:batchKey});
