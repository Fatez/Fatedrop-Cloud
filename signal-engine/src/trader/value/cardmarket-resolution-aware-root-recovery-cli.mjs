...
function assertExpectedRehearsal(report) {
  ...
  if(process.env.MAPPING_WRITE==='true' && (!expectedSectionB || !expectedSafe || !expectedCatalogue || !expectedGuide)) {
    throw new Error('Production mapping writes require pinned Section B count, safe mapping count, catalogue SHA and price-guide SHA');
  }
}
...
async function main() {
  ...
  try {
    ...
    assertExpectedRehearsal(report);
