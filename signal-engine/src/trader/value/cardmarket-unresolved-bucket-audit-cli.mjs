...
async function audit(db){
  ...
  for(const s of setRows){
    ...
    const expansionId=Number(source?.cardmarketExpansionId);
  ...
  for(const i of ids){
    ...
    if(!set.tcgdexSetId) bucket='missing_tcgdex_set_mapping';
      ...
      if(!expansion.length) bucket='cardmarket_expansion_absent_from_catalogue';
        ...
        let collector=null;
        try{collector=normaliseCollectorNumber(i.collector_number);}catch{}
        if(!collector) bucket='canonical_collector_invalid';
          ...
          if(!parsable.length) bucket='expansion_products_unparseable';
            ...
            if(both.length===1){
              bucket='exact_structured_should_have_matched';
              detail=both[0].product.sourceRecordId;
            } else if(both.length>1) {
              bucket='multiple_exact_structured_products';
              detail=both.map(x=>x.product.sourceRecordId);
            } else if(exactCollector.length===1 && exactName.length===0) {
              bucket='collector_exact_name_mismatch_provider_naming_candidate';
              detail={productId:exactCollector[0].product.sourceRecordId,productName:exactCollector[0].product.name};
            } else if(exactName.length===1 && exactCollector.length===0) {
              bucket='name_exact_collector_mismatch_numbering_candidate';
              detail={productId:exactName[0].product.sourceRecordId,productName:exactName[0].product.name,sourceCollector:exactName[0].parsed.collectorNumber};
            } else if(exactCollector.length>1) {
              bucket='collector_matches_multiple_products';
              detail=exactCollector.map(x=>({id:x.product.sourceRecordId,name:x.product.name}));
            } else if(exactName.length>1) {
              bucket='name_matches_multiple_products';
              detail=exactName.map(x=>({id:x.product.sourceRecordId,name:x.product.name,collector:x.parsed.collectorNumber}));
            } else {
  ...
  const byBucket=countBy(rows,r=>r.bucket);
  const byVariant=countBy(rows,r=>r.variantCode);
  const bySet=countBy(rows,r=>r.setName||r.setId);
  const recoverableBuckets=[
    'collector_exact_name_mismatch_provider_naming_candidate',
    'name_exact_collector_mismatch_numbering_candidate',
    'unparsed_or_provider_suffix_candidate',
    'exact_structured_should_have_matched'
  ];
  ...
  return {
    ...
    counts:{unresolvedNormalHolo:rows.length,recoverableCandidates:recoverableCandidates.length},
    byBucket,byVariant,topSets:Object.fromEntries(Object.entries(bySet).slice(0,30)),
...
async function main(){
  ...
  try{report=await audit(db);}
    ...
    const out=(process.env.RUNNER_TEMP||'.')+'/cardmarket-unresolved-bucket-audit.json';
    await writeFile(out,JSON.stringify(report,null,2));
    console.log(JSON.stringify({status:report.status,counts:report.counts,byBucket:report.byBucket},null,2));
  }
}
