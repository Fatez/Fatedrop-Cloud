...
async function build(db){
  ...
  return{
    ...
    counts,reasons,byVariant,bySet,candidates,
  };
}
  ...
  try{
    for(const row of report.candidates){
      ...
      if(source.rows[0]&&source.rows[0].card_identity_id!==row.cardIdentityId){
        throw new Error(`Cardmarket source ownership changed for ${row.sourceRecordId}/${row.sourceVariantKey}`);
      }
      ...
      const canonical=await db.query(`
        ...
        [row.cardIdentityId,row.sourceVariantKey]);
      if(canonical.rows[0]&&String(canonical.rows[0].source_record_id)!==row.sourceRecordId){
        throw new Error(`Canonical Cardmarket mapping changed for ${row.cardIdentityId}/${row.sourceVariantKey}`);
      }
      ...
      await db.query(`
        ...
        [row.id,row.cardIdentityId,row.sourceRecordId,row.sourceVariantKey,row.sourceVersion,observedAt]);
    }
...
async function main(){
  ...
  try{
    ...
    db.release();
    await pool.end();
    await writeFile(`${process.env.RUNNER_TEMP||'.'}/cardmarket-tcgdex-explicit-baseline-finish-recovery.json`,JSON.stringify(report,null,2));
