...
import { buildUnresolvedFacetDiagnostics } from "./unresolved-facet-diagnostics.mjs";
...
const FACET_AUDIT_ROW_LIMIT = 5000;
...
function reliabilityDiagnostics({ orphanRows = [], freshnessRows = [], now }) {
  ...
  return {
    ...
    telemetryStoppedWhileSignalsContinue: recentSignals > 0
...
export function buildSignalHealthSummary({
  ...
  for (const row of deliveryRows) {
    ...
    else if (result === "skipped" && (detail === "disabled" || detail.startsWith("policy_"))) point.policySkipped += value;
  ...
  return {
    ...
    diagnostics: {
      ...
      discovery: discoveryDiagnostics(discoveryRows),
      unresolvedFacets: buildUnresolvedFacetDiagnostics(facetRows),
    },
  };
}
...
export async function loadSignalHealthSummary(store, { days = 7, now = Math.floor(Date.now() / 1000) } = {}) {
  ...
  const [detections, deliveries, latency, orphans, freshness, discovery, facetAudit, rawMonitors] = await Promise.all([
    ...
    pool.query(`SELECT s.id,s.retailer_id,s.retailer_name,s.title,s.evidence,s.detected_at,COUNT(*) OVER()::int AS facet_audit_total FROM fatedrop_signals s WHERE s.detected_at >= $1 AND s.state IN ('whisper','echo','manifested','vanished') AND ${publicSignalSqlFilter("s")} AND ${validVanishedSqlFilter("s")} ORDER BY s.detected_at DESC,s.id ASC LIMIT ${FACET_AUDIT_ROW_LIMIT}`, [day0]).catch(() => ({ rows: null })),
  ...
  return buildSignalHealthSummary({
    ...
    facetRows: facetAudit.rows,
    monitorRows: rawMonitors,
    days: safeDays,
    now,
  });
}
