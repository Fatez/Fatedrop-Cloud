...
test('active-unpriced Cardmarket recovery manifest stays exact and fail-closed', async () => {
  ...
  assert.equal(manifest.expectedCandidates, 24);
  assert.equal(manifest.candidates.length, 24);
  assert.equal(new Set(manifest.candidates.map(row => row.cardIdentityId)).size, 24);
  assert.equal(new Set(manifest.candidates.map(row => `${row.sourceRecordId}|${row.sourceVariantKey}`)).size, 24);
  ...
  assert.ok(manifest.candidates.some(row => row.cardIdentityId === 'fdcard_043a2f0fc48bc2f9afaf7be5' && row.sourceRecordId === '568801'));
