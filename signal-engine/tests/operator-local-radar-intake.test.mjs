...
const NOW = Date.parse("2026-08-28T01:00:00+01:00");
...
function operatorIssue(overrides = {}) {
  const body = {
    ...
    notificationDateLabel: "tomorrow, 29 August",
    expiresAt: "2026-08-30T23:59:59+01:00",
    confidence: 0.68,
    targetBranches: [
  ...
  return {
    number: 301,
    state: "open",
    title: "[FATEDROP LOCAL RADAR] Mega Forces tins · The Entertainer · 29 Aug",
    body: JSON.stringify(body),
    created_at: "2026-08-28T00:55:00+01:00",
    updated_at: "2026-08-28T00:55:00+01:00",
    user: { login: "Fatez" },
    ...overrides,
    body: JSON.stringify(body),
  };
...
test("authorised official preparation evidence remains advisory Echo and builds descriptive copy", () => {
  ...
  assert.equal(
    notification.body,
    "Pokémon TCG: Mega Forces Tin (Styles Vary) expected at 2 The Entertainer stores tomorrow, 29 August. Check Local Radar to see if a participating store is near you.",
  );
});
...
test("operator intake rejects untrusted authors, pull requests and branchless broadcasts", () => {
  assert.throws(
    () => parseOperatorIssue(operatorIssue({ user: { login: "someone-else" } }), NOW),
    /author is not authorised/,
  );
  assert.throws(
    () => parseOperatorIssue(operatorIssue({ pull_request: { url: "https://example.invalid" } }), NOW),
    /Pull requests are not operator alerts/,
  );
  assert.throws(
    () => parseOperatorIssue(operatorIssue({ body: { targetBranches: [] } }), NOW),
    /At least one named target branch is required/,
  );
});
...
test("operator issue persists Expected branch intelligence before publishing through the Web bridge", async () => {
  const saved = [];
  let outbound = null;
  const store = {
  ...
  const fetchImpl = async (url, options) => {
    ...
    return new Response(JSON.stringify({ queued: 2, sent: 2 }), { status: 200, headers: { "content-type": "application/json" } });
  };
  ...
  try {
    ...
    for (const observation of saved) {
      ...
      assert.equal(observation.evidence.evidenceLevel, "inventory_preparation");
    }
    ...
    assert.match(payload.body, /Check Local Radar to see if a participating store is near you\.$/);
  } finally {
