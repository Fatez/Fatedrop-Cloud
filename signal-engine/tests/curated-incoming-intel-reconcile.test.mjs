...
const LOCATIONS = [
  { id: "loc-watford", retailerId: "entertainer-uk", name: "The Entertainer", address: "Atria Watford, Watford", postcode: "WD17 2UB", storeFormat: "toy_store" },
  { id: "loc-basildon", retailerId: "entertainer-uk", name: "The Entertainer Basildon", address: "Eastgate, Basildon", postcode: "SS14 1AF", storeFormat: "toy_store" },
  { id: "loc-other", retailerId: "entertainer-uk", name: "The Entertainer", address: "High Street, Oxford", postcode: "OX1 1AA", storeFormat: "toy_store" },
  { id: "loc-tesco", retailerId: "tesco-uk", name: "Tesco Extra Watford", address: "Watford", postcode: "WD17 1AA", storeFormat: "supermarket" },
];
...
test("curated official incoming evidence becomes branch-specific advisory Echo only for uniquely matched public branches", async () => {
  const saved = [];
  const store = {
  ...
  for (const row of saved) {
    ...
    assert.equal(row.evidence.exactBranch, true);
    assert.equal(row.evidence.chainWide, false);
    assert.equal(row.evidence.explicitTcgRelevance, true);
...
test("chain-wide evidence cannot make branches Echo eligible", async () => {
  const saved = [];
  const store = {
    async listRetailerLocations() { return LOCATIONS; },
    async upsertLocalStockObservations(rows) { saved.push(...rows); return { saved: rows.length, duplicates: 0 }; },
  ...
  const result = await reconcileCuratedIncomingIntel({
    store,
    entries: [entry({ explicitTcgRelevance: false, targetBranches: ["The Entertainer Watford"] })],
    now: Date.parse("2026-08-26T16:00:00+01:00"),
  });
  assert.equal(result.saved, 0);
  assert.equal(saved.length, 0);
  assert.equal(result.unmatchedTargets[0].reason, "location_not_echo_eligible");
});
...
test("unmatched or ambiguous target branches fail closed instead of becoming chain-wide intelligence", async () => {
  ...
  const store = {
    async listRetailerLocations() {
      return [
        ...LOCATIONS,
        { id: "loc-watford-2", retailerId: "entertainer-uk", name: "The Entertainer", address: "Another Centre, Watford", postcode: "WD18 0AA", storeFormat: "toy_store" },
      ];
    },
    async upsertLocalStockObservations(rows) { saved.push(...rows); return { saved: rows.length, duplicates: 0 }; },
  };
...
test("expired curated evidence loses Echo authority without creating a lifecycle event", async () => {
  let writes = 0;
  const store = {
    async listRetailerLocations() { return LOCATIONS; },
...
test("production curated record is time-bounded official preparation evidence, never a stock claim", () => {
  ...
  assert.equal(real.explicitTcgRelevance, true);
