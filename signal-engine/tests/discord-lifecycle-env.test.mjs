...
test("legacy Nixon token remains a fallback until Railway is renamed", () => {
  const discord = readDiscordEnv({
    DISCORD_NIXON_BOT_TOKEN: "legacy-nixon-token",
    DISCORD_VANISHED_CHANNEL_ID: "444",
  });
  assert.equal(discord.botTokens.vanished, "legacy-nixon-token");
});

test("canonical Nyxen token wins when both names are configured", () => {
  const discord = readDiscordEnv({
    DISCORD_NYXEN_BOT_TOKEN: "nyxen-token",
    DISCORD_NIXON_BOT_TOKEN: "legacy-nixon-token",
    DISCORD_VANISHED_CHANNEL_ID: "444",
  });
  assert.equal(discord.botTokens.vanished, "nyxen-token");
});
