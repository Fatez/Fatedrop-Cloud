  ...
  try {
    ...
    await local.query(`INSERT INTO fatedrop_tcgs VALUES ('pokemon','pokemon','Pokemon','active',1,1);
      ...
      INSERT INTO fatedrop_card_identities (id,canonical_key,tcg_id,series_id,set_id,printing_id,collector_number,variant_code,language_code,verification_status,verified_at,created_at,updated_at)
        ...
        FROM (SELECT n,CASE WHEN n<=27624 THEN 1+((n-1)%19087) ELSE 19088+((n-27625)%936) END p FROM generate_series(1,27725) n) x;
      INSERT INTO fatedrop_card_source_mappings (id,card_identity_id,source_name,source_record_id,source_variant_key,first_observed_at,last_observed_at)
        SELECT 'm'||n,'c'||n,'fixture','r'||n,'standard',1,1 FROM generate_series(1,27725) n;`);
    ...
    assert.deepEqual(saved,{verified_sets:165,printings:20023,verified_identities:27725,source_mappings:27725,orphan_sets:0,orphan_printings:0,orphan_mappings:0,duplicate_identities:0});
    ...
    assert.equal(check.expected.verified_identities,27725);
    assert.equal(check.expected.printings,20023);

    // A production-only natural-key collision must block the whole activation.
    await production.query("INSERT INTO fatedrop_card_source_mappings (id,card_identity_id,source_name,source_record_id,source_variant_key,first_observed_at,last_observed_at) VALUES ('collision','c1','fixture','r27725','standard',1,1)");
    ...
    assert.equal(report.after.verified_identities,27725);
    assert.equal(report.after.source_mappings,27726); // 27,725 rehearsal mappings + retained Cardmarket mapping.
