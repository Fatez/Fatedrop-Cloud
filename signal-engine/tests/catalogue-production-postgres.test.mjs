  ...
  try {
    ...
    await local.query(`INSERT INTO fatedrop_tcgs VALUES ('pokemon','pokemon','Pokemon','active',1,1);
      ...
      INSERT INTO fatedrop_card_sets (id,tcg_id,series_id,code,name,created_at,updated_at,verification_status,verified_at)
        SELECT 's'||n,'pokemon','series','s'||n,'Set '||n,1,1,'verified',1 FROM generate_series(1,157) n;
      INSERT INTO fatedrop_card_printings (id,tcg_id,series_id,set_id,printing_code,collector_number,name,created_at,updated_at,verification_status,verified_at)
        SELECT 'p'||n,'pokemon','series',
          CASE WHEN n<=18250 THEN 's'||(1+((n-1)%138)) ELSE 's'||(139+((n-18251)%19)) END,
          'p'||n,n::text,'Card '||n,1,1,'verified',1 FROM generate_series(1,19087) n;
      INSERT INTO fatedrop_card_identities (id,canonical_key,tcg_id,series_id,set_id,printing_id,collector_number,variant_code,language_code,verification_status,verified_at,created_at,updated_at)
        SELECT 'c'||n,'c'||n,'pokemon','series',
          CASE WHEN p<=18250 THEN 's'||(1+((p-1)%138)) ELSE 's'||(139+((p-18251)%19)) END,
          'p'||p,p::text,'fixture-'||n,'en','verified',1,1,1
        FROM (SELECT n,CASE WHEN n<=26169 THEN 1+((n-1)%18250) ELSE 18251+((n-26170)%837) END p FROM generate_series(1,27624) n) x;
      INSERT INTO fatedrop_card_source_mappings (id,card_identity_id,source_name,source_record_id,source_variant_key,first_observed_at,last_observed_at)
        SELECT 'm'||n,'c'||n,'fixture','r'||n,'standard',1,1 FROM generate_series(1,27624) n;`);
    for (const table of Object.keys(TABLES)) {
      let rows=(await local.query(`SELECT * FROM ${table}`)).rows;
      if(table==='fatedrop_card_sets') rows=rows.filter(r=>Number(r.id.slice(1))<=138);
      if(table==='fatedrop_card_printings') rows=rows.filter(r=>Number(r.id.slice(1))<=18250);
      if(table==='fatedrop_card_identities') rows=rows.filter(r=>Number(r.id.slice(1))<=26169);
      if(table==='fatedrop_card_source_mappings') rows=rows.filter(r=>Number(r.id.slice(1))<=26169);
    ...
    assert.equal(check.expected.verified_identities,27624);
    await production.query("INSERT INTO fatedrop_card_source_mappings VALUES ('collision','c1','fixture','r27624','standard',NULL,NULL,1,1)");
    await assert.rejects(activate({production,local,evidence,activate:true,report:{}}),/unique|duplicate/);
    assert.equal((await recount(production)).verified_identities,26169);
    await production.query("DELETE FROM fatedrop_card_source_mappings WHERE id='collision'");
    await production.query(`CREATE FUNCTION reject_new_fixture() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN IF NEW.id='s139' THEN RAISE EXCEPTION 'fixture rollback'; END IF; RETURN NEW; END $$;
