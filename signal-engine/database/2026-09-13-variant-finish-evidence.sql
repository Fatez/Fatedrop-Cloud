...
CREATE INDEX IF NOT EXISTS fatedrop_variant_evidence_snapshots_identity_time_idx ON fatedrop_variant_evidence_snapshots(card_identity_id, observed_at DESC);
CREATE INDEX IF NOT EXISTS fatedrop_variant_evidence_snapshots_provider_idx ON fatedrop_variant_evidence_snapshots(provider, observed_at DESC);

CREATE OR REPLACE FUNCTION fatedrop_reject_variant_evidence_snapshot_mutation() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN RAISE EXCEPTION 'variant evidence snapshots are immutable'; END;
$$;
DROP TRIGGER IF EXISTS fatedrop_variant_evidence_snapshots_immutable ON fatedrop_variant_evidence_snapshots;
CREATE TRIGGER fatedrop_variant_evidence_snapshots_immutable BEFORE UPDATE OR DELETE ON fatedrop_variant_evidence_snapshots FOR EACH ROW EXECUTE FUNCTION fatedrop_reject_variant_evidence_snapshot_mutation();
...
CREATE INDEX IF NOT EXISTS fatedrop_variant_evidence_reviews_identity_idx ON fatedrop_variant_evidence_reviews(card_identity_id, finish, approval_state);
...
CREATE INDEX IF NOT EXISTS fatedrop_variant_resolution_state_state_idx ON fatedrop_variant_resolution_state(classifier_state, updated_at DESC);
...
CREATE UNIQUE INDEX IF NOT EXISTS fatedrop_catalogue_audit_flags_active_unique ON fatedrop_catalogue_audit_flags(card_identity_id, flag_type) WHERE active = TRUE;
